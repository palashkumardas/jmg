        ...
        <div class="card">
           <div class="card-body table-responsive">
                <table class="mb-0 table table-bordered table-striped" id="data_table">
                    ...
                    <tbody>
                        @foreach($payout_partners as $list)
                        <tr>
                            <th>{{ ($payout_partners->currentPage()-1) * $payout_partners->perPage() + $loop->index + 1 }}</th>
                            <td>{{ $list->title }} </td>
                            <td>{{ $list->description }}</td>
                            <td><img src="{{ asset($list->picture) }}" alt="{{ $list->title }}" width="100" height="30"></td>
                            <td>
                               <a href="{{ route('payout-partner.edit',$list->id)}}" data-toggle="modal" data-target="#editModal{{ $list->id }}" class="btn btn-outline-info" title="Edit"><i class="fa fa-pencil"></i></a>
                          ...
                          <div class="modal-dialog" role="document">
                            <div class="modal-content">
                              ...
                              <div class="modal-footer">
                                ...
                                <form method="POST" action="{{ url('admin/payout-partner/'.$list->id) }}">
                                   @csrf
                                   @method('DELETE') 
                        ...
                        <div class="modal fade" id="editModal{{ $list->id }}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
                          <div class="modal-dialog" role="document">
                            <div class="modal-content">
                              ...
                              <div class="modal-body">
                                <form method="post" action="{{ url('admin/payout-partner/'.$list->id) }}" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    <input type="hidden" name="old_img" value="{{ $list->id }}">
            ...
            {{ $payout_partners->links() }}
        </div>
    </div>
    <div class="col-4">
        <div class="card">
            <div class="card-body table-responsive">
                <form method="POST" action="{{ route('payout-partner.store') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label for="title">Title</label>
